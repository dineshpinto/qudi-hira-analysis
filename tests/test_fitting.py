...
class TestFitting(TestCase):
    ...
    def test_pulsedodmr_lorentziandouble_fit(self):
        ...
        x_fit, y_fit, _ = self.dh.fit(x="Controlled variable(Hz)", y="Signal", data=odmr.data,
    ...
    def test_pusledrabi_sineexponentialdecay_fit(self):
        ...
        x_fit, y_fit, _ = self.dh.fit(x="Controlled variable(s)", y="Signal", data=rabi.data,
    ...
    def test_saturation_hyperbolicsaturation_fit(self):
        sat = self.dh.read_excel(self.dh.data_folder_path / "saturation_curve.xlsx", skiprows=1)
        x_fit, y_fit, _ = self.dh.fit(x="Power", y="Counts", data=sat,
