...
def calc_transitions(E, B, D):
    a1 = np.sort(np.linalg.eig(NV_Hamiltonian(E, B, 2.87e9)[0])[
...
def calc_P1_transitions(B):
    a1 = np.sort(np.linalg.eig(P1_Hamiltonian(B)[0])[
                     0].real)  # eig[0] is the array of eigenvalues (eig[1] = the diagonal matrix)
...
def find_B_NV(f_transitions):
    ...
    # x0 = E, B, D
    x0 = [0.0,
          0.1 * (2.87000e9 - f_transitions[0]) / 28e9, 0.1 * (2.87000e9 - f_transitions[0]) / 28e9,
          (2.8700e9 - f_transitions[0]) / 28e9,
          2.87e9]  # x0: initial guess, here:  E=0, Bx, By = 0.1*Bz, D=2.88e9
    NV_values = optimize.root(f_NV, x0,
                              method='lm')  # 'lm': Solve for least squares with Levenberg-Marquardt. Without this an error message will be displayed.
    # print(NV_values['message'])
    # return NV_values['x'], absoluteValue([NV_values['x'][1], NV_values['x'][2], NV_values['x'][3]])

    B = NV_values['x']
    result = {"B_abs": absoluteValue(B[1:4]), "Bx": B[1], "By": B[2], "Bz": B[3],
              "theta_z_degrees": angle(B[1:4], (0, 0, 1))}
    return result
...
def find_B_P1(f_transitions):
    ...
    # x0 = B
    x0 = [0.1 * f_transitions[1] / 28e9, 0.1 * f_transitions[1] / 28e9,
          f_transitions[1] / 28e9]  # x0: initial guess, here:  Bx, By = 0.1*Bz, Bz = central transition 1
    P1_values = optimize.root(f_P1, x0,
                              method='lm')  # 'lm': Solve for least squares with Levenberg-Marquardt. Without this an error message will be displayed.
