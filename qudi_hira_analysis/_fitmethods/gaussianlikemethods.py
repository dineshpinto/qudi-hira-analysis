...
def make_gaussianwithoutoffset_model(self, prefix=None):
    ...
    full_gaussian_model.set_param_hint(
        f"{prefix!s}fwhm", expr=f"2.3548200450309493*{prefix}sigma"
    )
...
def make_multiplegaussianoffset_model(self, no_of_functions=1):
    ...
    if no_of_functions == 1:
        ...
        for ii in range(1, no_of_functions):
            prefix = f"g{ii:d}_"
            multi_gaussian_model += self.make_gaussianwithoutoffset_model(
                prefix=prefix
            )[0]
            multi_gaussian_model.set_param_hint(
                f"{prefix}contrast", expr=f"({prefix}amplitude/offset)*100"
...
def make_twoDgaussian_model(self, prefix=None):  # noqa: N802
    ...
    if not isinstance(prefix, str) and prefix is not None:
        ...
        gaussian_2d_model = Model(
            twoDgaussian_function, independent_vars=["x"], prefix=prefix
        )
...
def estimate_gaussian_peak(self, x_axis, data, params):
    ...
    params["center"].set(
        value=x_axis[np.argmax(data_smoothed)], min=center_min, max=center_max
    )
    ...
    params["sigma"].set(
        value=np.sqrt(abs(mom2 - mean_val_calc**2)), min=sigma_min, max=sigma_max
    )
    ...
    params["amplitude"].set(
        value=data_smoothed.max() - data_smoothed.min(), min=ampl_min
    )
...
def estimate_gaussian_dip(self, x_axis, data, params):
    ...
    params["amplitude"].set(
        value=-params_ret["amplitude"].value, min=-np.inf, max=1e-12
    )
...
def estimate_twoDgaussian(self, x_axis, y_axis, data, params):  # noqa: N802
    ...
    params["sigma_x"].set(
        value=sigma_x, min=1 * stepsize_x, max=3 * (x_axis[-1] - x_axis[0])
    )
    params["sigma_y"].set(
        value=sigma_y, min=1 * stepsize_y, max=3 * (y_axis[-1] - y_axis[0])
    )
    params["center_x"].set(
        value=center_x,
