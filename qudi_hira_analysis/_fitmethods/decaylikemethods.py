...
def make_barestretchedexponentialdecay_model(self, prefix=None):
    ...
    if not isinstance(prefix, str) and prefix is not None:
        ...
        model = Model(
            barestretchedexponentialdecay_function,
            independent_vars=["x"],
            prefix=prefix,
        )
...
def make_biexponential_model(self, prefix=None):
    ...
    exponential_model = (
        amp0_model * exp0_model + amp1_model * exp1_model + constant_model
    )
...
def estimate_decayexponential(self, x_axis, data, params):
    ...
    try:
        ...
        params["lifetime"].set(
            value=-1 / linear_result.params["slope"].value, min=min_lifetime
        )
