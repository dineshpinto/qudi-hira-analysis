...
def make_sinestretchedexponentialdecay_model(self, prefix=None):
    ...
    sine_model, params = self.make_sinewithoutoffset_model(prefix=prefix)
    (
        bare_stretched_exp_decay_model,
        params,
    ) = self.make_barestretchedexponentialdecay_model(prefix=prefix)
    constant_model, params = self.make_constant_model(prefix=prefix)
...
def make_sinedoublewithtwoexpdecay_model(self, prefix=None):
    ...
    sinedoublewithtwoexpdecay = (
        sine_exp_decay_model1 + sine_exp_decay_model2 + constant_model
    )
...
def make_sinetriplewiththreeexpdecay_model(self, prefix=None):
    ...
    three_sine_exp_decay_offset = (
        sine_exp_decay_model1
        + sine_exp_decay_model2
        + sine_exp_decay_model3
        + constant_model
    )
    params = three_sine_exp_decay_offset.make_params()
...
def estimate_sinewithoutoffset(self, x_axis, data, params):
    ...
    for _tries, _diff_array_step in enumerate(diff_array):
        if np.isclose(min_x_diff, 0.0, atol=1e-12):
            ...
            if len(diff_array) == 0:
                self.log.error(
                    "The passed x_axis for the sinus estimation contains the "
                    "same values! Cannot do the fit!"
                )
...
def make_sine_fit(self, x_axis, data, estimator, units=None, add_params=None, **kwargs):
    ...
    try:
        period_err = (
            result.params["frequency"].stderr / (result.params["frequency"]) ** 2
        )
...
def estimate_sine(self, x_axis, data, params):
    ...
    error, params = self.estimate_sinewithoutoffset(
        x_axis=x_axis, data=data_level, params=params
    )
...
def make_sineexponentialdecay_fit(
    ...
    try:
        ...
        result = sine_exp_decay_offset.fit(data, x=x_axis, params=params, **kwargs)
        self.log.error(
            "The sineexponentialdecayoffset fit did not work.\n"
            f"Error message: {result.message}"
        )
...
def make_sinestretchedexponentialdecay_fit(
    ...
    try:
        ...
        result = sine_stretched_exp_decay.fit(data, x=x_axis, params=params, **kwargs)
        self.log.error(
            "The sineexponentialdecay fit did not work.\n"
            f"Error message: {result.message}"
        )
...
def estimate_sinestretchedexponentialdecay(self, x_axis, data, params):
    """Provide a estimation of a initial values for a sine stretched exponential
    decay function.
...
def make_sinedouble_fit(
    ...
    try:
        ...
        self.log.warning(
            "The twosineexpdecayoffset fit did not work. "
    ...
    try:
        period1_err = (
            result.params["s1_frequency"].stderr / (result.params["s1_frequency"]) ** 2
        )
    except ZeroDivisionError:
        period1_err = np.inf
    ...
    try:
        period2_err = (
            result.params["s2_frequency"].stderr / (result.params["s2_frequency"]) ** 2
        )
    except ZeroDivisionError:
        period2_err = np.inf
...
def estimate_sinedouble(self, x_axis, data, params):
    ...
    result2 = self.make_sine_fit(
        x_axis=x_axis, data=data_sub, estimator=self.estimate_sine
    )
...
def make_sinedoublewithexpdecay_fit(
    ...
    try:
        ...
        self.log.warning(
            "The sinedoublewithexpdecay fit did not work. "
            f"Error message: {result.message!s}"
        )
        result = two_sine_exp_decay_offset.fit(data, x=x_axis, params=params, **kwargs)
...
def make_sinedoublewithtwoexpdecay_fit(
    ...
    params = self._substitute_params(initial_params=params, update_params=add_params)
    try:
        result = two_sine_two_exp_decay_offset.fit(
            data, x=x_axis, params=params, **kwargs
        )
    except:  # noqa: E722
        self.log.warning(
            "The sinedoublewithtwoexpdecay fit did not work. "
            f"Error message: {result.message!s}"
        )
        result = two_sine_two_exp_decay_offset.fit(
            data, x=x_axis, params=params, **kwargs
    ...
    try:
        period1_err = (
            result.params["e1_frequency"].stderr / (result.params["e1_frequency"]) ** 2
        )
    except ZeroDivisionError:
        period1_err = np.inf
    ...
    try:
        period2_err = (
            result.params["e2_frequency"].stderr / (result.params["e2_frequency"]) ** 2
        )
    except ZeroDivisionError:
        period2_err = np.inf
...
def make_sinetriple_fit(
    ...
    try:
        ...
        self.log.warning(
            "The threesineexpdecayoffset fit did not work. "
            f"Error message: {result.message!s}"
        )
        result = two_sine_offset.fit(data, x=x_axis, params=params, **kwargs)
    ...
    try:
        period3_err = (
            result.params["s3_frequency"].stderr / (result.params["s3_frequency"]) ** 2
        )
    except ZeroDivisionError:
        period3_err = np.inf
...
def estimate_sinetriple(self, x_axis, data, params):
    ...
    res2 = self.make_sine_fit(
        x_axis=x_axis, data=data_sub1, estimator=self.estimate_sine
    )
    data_sub2 = data_sub1 - res2.best_fit

    res3 = self.make_sine_fit(
        x_axis=x_axis, data=data_sub2, estimator=self.estimate_sine
    )
...
def make_sinetriplewithexpdecay_fit(
    ...
    params = self._substitute_params(initial_params=params, update_params=add_params)
    try:
        result = three_sine_exp_decay_offset.fit(
            data, x=x_axis, params=params, **kwargs
        )
    except:  # noqa: E722
        self.log.warning(
            "The sinetriplewithexpdecay fit did not work. "
            f"Error message: {result.message!s}"
        )
        result = three_sine_exp_decay_offset.fit(
            data, x=x_axis, params=params, **kwargs
...
def make_sinetriplewiththreeexpdecay_fit(
    ...
    """
    (
        three_sine_three_exp_decay_offset,
        params,
    ) = self.make_sinetriplewiththreeexpdecay_model()

    error, params = estimator(x_axis, data, params)

    params = self._substitute_params(initial_params=params, update_params=add_params)
    try:
        result = three_sine_three_exp_decay_offset.fit(
            data, x=x_axis, params=params, **kwargs
        )
    except:  # noqa: E722
        self.log.warning(
            "The twosinetwoexpdecayoffset fit did not work. "
            f"Error message: {result.message!s}"
        )
        result = three_sine_three_exp_decay_offset.fit(
            data, x=x_axis, params=params, **kwargs
    ...
    try:
        period3_err = (
            result.params["e3_frequency"].stderr / (result.params["e3_frequency"]) ** 2
        )
    except ZeroDivisionError:
        period3_err = np.inf
...
def estimate_sinetriplewiththreeexpdecay(self, x_axis, data, params):
    ...
    params["e1_lifetime"].set(
        value=res1.params["lifetime"].value, min=2 * (x_axis[1] - x_axis[0])
    )
    ...
    params["e2_lifetime"].set(
        value=res2.params["lifetime"].value, min=2 * (x_axis[1] - x_axis[0])
    )
    ...
    params["e3_lifetime"].set(
        value=res3.params["lifetime"].value, min=2 * (x_axis[1] - x_axis[0])
    )
